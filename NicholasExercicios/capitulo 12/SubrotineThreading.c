%{ // Subrotine Threading - consiste em uma série de instruções de "chamada" em linguagem de máquina . Os primeiros compiladores para ALGOL, Fortran, Cobol e alguns sistemas Forth frequentemente produziam código de sub-rotina. O código em muitos desses sistemas operava em uma pilha de operandos LIFO (last-in-first-out).
